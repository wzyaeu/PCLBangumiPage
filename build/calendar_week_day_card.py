from tool import gett, logs_add, escape_xaml, resources
...
def calendar_week_day_card_get(json_data, i):
    ...
    for index, item in enumerate(json_data[i]['items'], start=1):
        ...
        output += itemt
        if item.get('images',None) != None:
            if item['images'].get('common',None) != None:
                resources[f'fanimage_common_{item['id']}.jpg'] = item['images']['common']
            else:
                resources[f'fanimage_common_{item['id']}.jpg'] = item['images'][list(item['images'].keys)[0]]
